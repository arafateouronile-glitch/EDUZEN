...
export default function EditLessonPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/dashboard/elearning/courses/${courseSlug}`)
    },
    onError: (error: Error) => {
      addToast({
        type: 'error',
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Modifier la leçon</h1>
          <p className="text-gray-500 mt-1">Modifiez la leçon "{lesson.title}" du cours "{course.title}"</p>
        </div>
      </div>
                  ...
                  {block.type === 'media' && (
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          {block.data.mediaType === 'image'
                            ? "URL de l'image (ou laisser vide si uploadée ci-dessus)"
                            : 'URL du média'}
                        </label>
                        <input
                          type="url"
