...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!fileUrl || typeof fileUrl !== 'string' || fileUrl.trim() === '') {
      return NextResponse.json(
        { error: 'Document sans fichier PDF attaché' },
        { status: 404 }
      )
    }
    ...
    if (!path) {
      logger.error('document-pdf-url: URL non parseable', { fileUrl, supabaseUrl })
      return NextResponse.json(
        { error: 'Format d\'URL du document non supporté' },
        { status: 400 }
      )
    }

    // Retourner l’URL du proxy (même origine) pour éviter le blocage CORS
    // du viewer PDF avec les URLs signées Supabase.
    const url = `/api/sign/document-pdf?token=${encodeURIComponent(token)}`
