...
export function OrganizationHealthCard({ org }: OrganizationHealthCardProps) {
  ...
  return (
    <Link href={`/super-admin/crm/${org.organization_id}`}>
      <Card
        className={cn(
          'group cursor-pointer border-l-4 transition-all hover:shadow-md hover:-translate-y-0.5',
          health.border
        )}
      >
        <CardContent className="p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            {/* Arrow */}
            <ChevronRight className="h-4 w-4 flex-shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 mt-0.5" />
          </div>
        </CardContent>
      </Card>
    </Link>
