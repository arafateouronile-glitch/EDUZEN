...
import { POST } from '@/app/api/subscriptions/create-checkout/route'

vi.mock('@/lib/supabase/server', () => ({ createClient: vi.fn() }))

describe('API POST /api/subscriptions/create-checkout', () => {
  ...
  it('retourne 401 sans auth', async () => {
    ...
    const req = new NextRequest('http://localhost/api/subscriptions/create-checkout', {
      method: 'POST',
      body: JSON.stringify({ planId: 'plan-1', billingPeriod: 'monthly' }),
    })
