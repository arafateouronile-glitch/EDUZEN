...
export function buildCheckInEmail({ prenom }: { prenom: string }): string {
  return wrap(`
    ...
    <p style="margin:0 0 20px;">Si vous avez des questions, si quelque chose bloque, ou si vous voulez juste vous assurer que vous tirez le meilleur parti de l'application — je vous propose qu'on prenne <strong>10 minutes ensemble</strong>.</p>
    <p style="margin:0 0 20px;">Pas de présentation, pas de démo. Juste un appel rapide pour débloquer ce dont vous avez besoin.</p>
...
export function buildTrialEndingSoonEmail({ prenom, appUrl }: { prenom: string; appUrl: string }): string {
  return wrap(`
    ...
    <p style="margin:0 0 28px;">Et si vous êtes prêt à continuer :</p>
...
export function buildTrialLastDayEmail({ prenom, appUrl }: { prenom: string; appUrl: string }): string {
  return wrap(`
    ...
    <p style="margin:0 0 28px;">Ou si vous êtes prêt à continuer maintenant :</p>
    <p style="margin:0 0 28px;text-align:center;">
