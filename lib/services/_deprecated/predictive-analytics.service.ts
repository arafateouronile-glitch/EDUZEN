...
// Types locaux pour les tables prediction qui ne sont pas encore dans le schéma Supabase
type SuccessRatePrediction = any
type DropoutPrediction = any
type PredictionAlert = any
type StudentFeature = any
...
export class PredictiveAnalyticsService {
  ...
  async generateSuccessRateReport(
    ...
    const { data: predictions } = await (this.supabase as any)
      .from('success_rate_predictions')
      .select('*')
      .eq('organization_id', organizationId)
    ...
    const stats = {
      ...
      accurate_predictions: predictions.filter((p: any) => p.was_accurate).length,
      accuracy_rate: 0,
    ...
    if (predictions.length > 0) {
      ...
      stats.average_predicted_rate =
        predictions.reduce((sum: number, p: any) => sum + (p.predicted_success_rate || 0), 0) /
        predictions.length
      stats.average_actual_rate =
        predictions.reduce((sum: number, p: any) => sum + (p.actual_success_rate || 0), 0) /
        predictions.length
  ...
  async generateDropoutReport(
    ...
    const stats = {
      ...
      accurate_predictions: predictions.filter((p: any) => p.was_accurate).length,
      accuracy_rate: 0,
      actual_dropouts: predictions.filter((p: any) => p.did_dropout).length,
      dropout_rate: 0,
    ...
    if (predictions.length > 0) {
      ...
      stats.average_predicted_probability =
        predictions.reduce((sum: number, p: any) => sum + (p.dropout_probability || 0), 0) /
        predictions.length
