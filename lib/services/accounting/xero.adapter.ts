/**
 * Adapter pour Xero
 * Adapter Xero - Intégration comptable
 * 
 * NOTE: Adapter non implémenté - Fonctionnalité prévue pour une future version
 * Nécessite intégration avec l'API Xero (OAuth2, endpoints de synchronisation comptable)
 */
...
export class XeroAdapter implements AccountingAdapter {
  async authenticate(config: AccountingConfig, authCode?: string): Promise<{ access_token: string; refresh_token?: string; expires_in?: number }> {
    throw new Error('Xero adapter n\'est pas encore implémenté')
  }
