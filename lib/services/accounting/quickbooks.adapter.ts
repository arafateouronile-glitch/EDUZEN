/**
 * Adapter pour QuickBooks
 * Adapter QuickBooks - Intégration comptable
 * 
 * NOTE: Adapter non implémenté - Fonctionnalité prévue pour une future version
 * Nécessite intégration avec l'API QuickBooks (OAuth2, endpoints de synchronisation comptable)
 */
...
export class QuickBooksAdapter implements AccountingAdapter {
  async authenticate(config: AccountingConfig, authCode?: string): Promise<{ access_token: string; refresh_token?: string; expires_in?: number }> {
    throw new Error('QuickBooks adapter n\'est pas encore implémenté')
  }
