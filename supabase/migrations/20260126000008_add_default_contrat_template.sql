-- Migration: Ajouter un modèle de contrat de formation par défaut
-- Contrat entre l'organisme et l'apprenant (personne physique)
-- Conforme aux articles L.6353-3 à L.6353-7 du Code du travail

-- Fonction pour créer le template contrat pour une organisation
CREATE OR REPLACE FUNCTION create_default_contrat_template(org_id UUID)
RETURNS UUID AS $$
DECLARE
  template_id UUID;
BEGIN
  -- Vérifier si un template contrat par défaut existe déjà
...
-- Créer le template contrat pour toutes les organisations existantes qui n'en ont pas
DO $$
DECLARE
  org RECORD;
BEGIN
  FOR org IN SELECT id FROM organizations LOOP
    PERFORM create_default_contrat_template(org.id);
  END LOOP;
END $$;

-- Commentaire sur la fonction
COMMENT ON FUNCTION create_default_contrat_template(UUID) IS
'Crée un modèle de contrat de formation par défaut pour une organisation.
Contrat B2C entre l organisme et l apprenant (personne physique).
Conforme aux articles L.6353-3 à L.6353-7 du Code du travail.
Inclut:
- Identification des parties (organisme + stagiaire)
- Objet du contrat (formation, objectifs, programme, dates, lieu)
